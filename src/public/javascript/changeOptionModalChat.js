...
function modelToChangeModalChatGroup(idToChange){
  return `
      ...
      <a href="#" data-type-chat="chat-group" data-toggle="modal" data-target="#addFriendToGroup" data-navigation-target="add-member-to-group" class="dropdown-item add-member-to-group" data-uid="${idToChange}">Thêm thành viên</a>
  ...
  if(type === "chat-group"){
    ...
    $.ajax({
      ...
      success: function() {
        ...
        leaveGroupChat();
        addMemberToGroup();
        emptyModalAddMemberToGroupChat();
      },
